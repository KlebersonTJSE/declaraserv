...
#
# Este módulo não existia nos arquivos originais enviados
# (app2.R apenas chamava mod_totp_admin_ui/_server) e foi
# criado agora, reaproveitando as funções já prontas em
# R/auth_totp.R (cadastrar_usuario_totp, listar_usuarios_totp,
# desativar_usuario_totp).
# =====================================================
...
mod_totp_admin_server <- function(id, con, ativo) {
    ...
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(input$cadastrar, {
            ...
            resultado <- tryCatch(
                cadastrar_usuario_totp(con, login, nome),
                error = function(e) {
                    showNotification(
            ...
            if (!is.null(resultado)) {
                ...
                showNotification(
                    paste0("Chave TOTP gerada para '", login, "'."),
                    type = "message"
                )
            }
        })
        ...
        output$resultado_cadastro <- renderUI({
            ...
            div(
                ...
                tags$b("Chave gerada para: "), r$login, tags$br(),
